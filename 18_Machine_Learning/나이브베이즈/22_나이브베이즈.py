...
"""
1. 12_텍스트_데이터_분석에서
   WordCloud KoNPy 에서 형태소로 분리해서 단어 찾기 
...
스팸         자동       필터링           나이브 베이즈
감정 분석(긍정/부정 자동 분류)           나이브 베이즈
뉴스   카테고리   자동    분류           나이브 베이즈
...
2. KNN 선형회귀, 로지스틱, 나이브베이즈 정리

                      KNN                선형회귀           로지스틱       나이브 베이즈
목          적     분류/예측             숫자예측             분류              분류
결    과    물    클래스/숫자          연속된 숫자          확률(0~1)          클래스
언제사용하는가    가까운 이웃       결과로 직선그리기     확률로 나누기      조건부 확률
데이터 량 확인    많을수록 좋다.          정도껏             정도껏          적어도 된다

코드를 어떻게 쓰는가
모델에는 논문 / 깃허브 모델을 사용하는 방법 코드 올려놓는다.
방법 코드를 참고해서 초기에 모델을 이용해보고 각 회사와 개발자가 계산한 수학 값으로 모델에 전처리 계산을 다르게 하여 사용

데이터 작업 / 컬럼 작업 -> 전처리
전처리 제일 어려움
-> 앞서 작성한 train_test_split 언제 fit_transform transform 사용하고 
어떤 컬럼을 선택해서 사용하고 와 같이 언제 무엇을 전처리 해야하는가 에 대한 정의를 내린 다는 것 굉장히 어렵다.
회사가 원하는 결과에 도달하기 위하여 굉장히 많은 숫자값을 테스트하며 정확도를 높여야한다.
